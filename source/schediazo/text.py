...
class Text(TextRendering,Stroke,Fill,Transform,Clip,Styling,Font,PartBase):
    ...
    def __init__(self, text: str, x: pint.Quantity=None, y: pint.Quantity=None, dx: pint.Quantity=None, dy: pint.Quantity=None,
        ...
        if x is not None:
            if not isinstance(x,(pint.Quantity)):
                raise TypeError
            if not (x.check('[length]') or x.check('[pixel]') or x.check('[percentage]')):
                raise ValueError
        if y is not None:
            if not isinstance(y,(pint.Quantity)):
                raise TypeError
            if not (y.check('[length]') or y.check('[pixel]') or y.check('[percentage]')):
                raise ValueError
        if dy is not None:
            if not isinstance(dy,(pint.Quantity)):
                raise TypeError
            if not (dy.check('[length]') or dy.check('[pixel]') or dy.check('[percentage]')):
                raise ValueError
        if dx is not None:
            if not isinstance(dx,(pint.Quantity)):
                raise TypeError
            if not (dx.check('[length]') or dx.check('[pixel]') or dx.check('[percentage]')):
                raise ValueError
        if rotate is not None:
            for angle in rotate:
                if isinstance(rotate,pint.Quantity):
                    if not rotate.check('[angle]'):
                        raise ValueError("rotation elements must be a pint.Quantity angle or a string")
                if not isinstance(rotate,(pint.Quantity,str)):
                    raise ValueError("rotation elements must be a pint.Quantity angle or a string")
        self._text = text
        self._x = x
        self._y = y
    ...
    def set_element_attributes(self, element: Union[ET.Element,ET.SubElement], 
        ...
        if self._dx is not None:
            element.set('dx', _tostr(self._dx))
        if self._dy is not None:
            element.set('dy', _tostr(self._dy))
        if self._rotate is not None:
            ...
            for r in self._rotate:
                if type(r,str):
                    ...
                    s.join('{} '.format(r.to(ureg.deg).magnitude))
            element.set('rotate', s)

        super(Text, self).set_element_attributes(element, device_per_length=device_per_length, device_per_pixel=device_per_pixel)
...
class TextPath(TextRendering,Stroke,Fill,Transform,Clip,Styling,PartBase):
    ...
    def set_element_attributes(self, element: Union[ET.Element,ET.SubElement], 
        ...
        super(TextPath, self).set_element_attributes(element, device_per_length=device_per_length, device_per_pixel=device_per_pixel)
